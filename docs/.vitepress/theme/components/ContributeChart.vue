<!-- 归档页顶部贡献图：按文章日期绘制的近一年热力图（echarts calendar heatmap）
     参考 https://vp.teek.top/resources/expand/contribute-chart.html -->
...
const contributeList = computed<[string, number][]>(() => {
  ...
  return contributeDays.map((item): [string, number] => [item, contributeObject[item] ?? 0]).reverse();
});
